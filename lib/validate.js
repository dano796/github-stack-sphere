...
const VALID_THEMES = new Set(["dark", "light", "auto", "transparent"]);
const VALID_DIRECTIONS = new Set(["cw", "ccw"]);
...
export function validateParams(query) {
  ...
  const direction = VALID_DIRECTIONS.has(query.direction)
    ? query.direction
    : "cw";

  const bg = parseHex(query.bg);
  const iconColor = parseHex(query.iconColor);

  return { slugs, theme, speed, size, direction, bg, iconColor };
}
