...
// `ccw` negates AXIS.y instead of reversing the animation. SVG screen-y points
// down, so flipping the axis's Y component mirrors the horizontal sense of the
// spin while keeping the vertical sense intact — front icons keep descending
// either way, they just enter the visible face from the opposite side.
function buildAxis(direction) {
  const ySign = direction === "ccw" ? -1 : 1;
  return {
    x: Math.sin(AXIS_TILT), // ≈ 0.574
    y: ySign * Math.cos(AXIS_TILT), // ≈ ±0.819
    z: 0,
  };
}
 ...
 */
function projectPoint(point, angle, cx, cy, sphereRadius, axis) {
...
export function computeKeyframes(
  spherePoints,
  { speed = 30, size = 500, direction = "cw" },
) {
  const width = size;
  ...
  // Per-icon animation rules — direction is baked into the rotation axis
  // (see buildAxis), so the CSS animation always plays forward.
