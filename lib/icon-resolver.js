 ...
 */
export function resolveIcons(slugs, theme, iconColor = null) {
  const resolved = [];
  const skipped = [];

  // `transparent` borrows `auto`'s dual-variant color logic so icons stay
  // visible whether GitHub renders the SVG over a dark or light page.
  const colorTheme = theme === "transparent" ? "auto" : theme;
  ...
  for (const slug of slugs) {
    ...
    resolved.push({
      slug: icon.slug,
      title: icon.title,
      color: iconColor ? iconColor : computeIconColor(icon.hex, colorTheme),
      path: icon.path,
    });
  }
