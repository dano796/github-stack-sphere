...
export function buildSVG({
  ...
  const symbols = icons
    .map((icon) => {
      ...
      // auto + transparent both ship dual dark/light color vars so the icon
      // adapts to the surrounding page; uniform iconColor overrides return a
      // plain string and fall through to the static fill branch.
      const usesColorVars =
        (theme === "auto" || theme === "transparent") &&
        typeof icon.color === "object";

      const fillAttr = usesColorVars
  ...
  if (theme === "auto") {
    ...
    // Backgrounds match GitHub's actual page color in each mode so the SVG
    // blends seamlessly into the README — #0d1117 (dark canvas) and #ffffff
    // (light canvas). #f6f8fa is GitHub's card/code-block tone, not the page.
    bgCss = `
  :root { --bg: #0d1117; --sphere-stroke: rgba(255,255,255,0.06); }
  @media (prefers-color-scheme: light) {
    :root { --bg: #ffffff; --sphere-stroke: rgba(0,0,0,0.06); }
  }
  .bg  { fill: var(--bg); }
  .sph { stroke: var(--sphere-stroke); }`;
  }

  if (theme === "auto" || theme === "transparent") {
    // Per-icon CSS custom properties — only meaningful for icons whose
    // resolver returned an object (i.e. uniform iconColor wasn't used).
    const dualIcons = icons.filter((i) => typeof i.color === "object");

    if (dualIcons.length > 0) {
      const darkVars = dualIcons
        .map((i) => `    --c-${i.slug}: #${i.color.dark};`)
        .join("\n");
      const lightVars = dualIcons
        .map((i) => `    --c-${i.slug}: #${i.color.light};`)
        .join("\n");
  ...
  if (theme !== "transparent") {
    const bgAttrs = bg
      ...
      : theme === "auto"
        ? 'class="bg"'
        : `fill="${themeConfig.bg}"`;
    bgRect = `  <rect ${bgAttrs} width="${width}" height="${height}" rx="12"/>`;
  }
